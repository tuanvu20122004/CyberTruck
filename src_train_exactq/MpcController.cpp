...
ExactQResult MpcController::solveQPExactQWithGradient(const Eigen::VectorXd& x0,
                                                      const Eigen::VectorXd& v_k,
                                                      double u0_fixed_rad) {
    ExactQResult result;
    ...
    if (x0.size() != nx || v_k.size() != N_) {
        std::cerr << "[MPC][ExactQGrad] Invalid input sizes!" << std::endl;
        return result;
    }
    ...
    if (!G.allFinite() || !Aeq.allFinite() || !beq.allFinite()) {
        std::cerr << "[MPC][ExactQGrad] Matrix contains NaN/Inf!" << std::endl;
        return result;
    }
    ...
    const int u0_index = dyn_constraint_dim;
    const int fixed_u_constraint_index = dyn_constraint_dim;
    Aall(fixed_u_constraint_index, u0_index) = 1.0;

    Eigen::VectorXd lb(total_constraint_dim);
    Eigen::VectorXd ub(total_constraint_dim);
    lb.head(dyn_constraint_dim) = beq;
    ub.head(dyn_constraint_dim) = beq;
    lb(fixed_u_constraint_index) = u0_fixed_rad;
    ub(fixed_u_constraint_index) = u0_fixed_rad;
    ...
    if (!exact_solver.data()->setHessianMatrix(G_sparse) ||
        ...
        !exact_solver.data()->setLowerBound(lb) ||
        !exact_solver.data()->setUpperBound(ub)) {
        std::cerr << "[MPC][ExactQGrad] Failed to set solver data!" << std::endl;
        return result;
    }

    if (!exact_solver.initSolver()) {
        std::cerr << "[MPC][ExactQGrad] Failed to init solver!" << std::endl;
        return result;
    }

    if (exact_solver.solveProblem() != OsqpEigen::ErrorExitFlag::NoError) {
        std::cerr << "[MPC][ExactQGrad] Solve failed!" << std::endl;
        return result;
    }

    Eigen::VectorXd z_opt = exact_solver.getSolution();
    Eigen::VectorXd dual = exact_solver.getDualSolution();

    if (z_opt.size() != z_dim || !z_opt.allFinite() ||
        dual.size() != total_constraint_dim || !dual.allFinite()) {
        std::cerr << "[MPC][ExactQGrad] Invalid primal/dual solution!" << std::endl;
        return result;
    }

    const Eigen::VectorXd Hz = G * z_opt;
    result.objective = 0.5 * z_opt.dot(Hz) + g.dot(z_opt);

    // OSQP KKT convention: Pz + q + A^T y = 0.
    // Our equality row is A_u z - u_fixed = 0.
    // Envelope theorem gives dQ / d(u_fixed_rad) = -y_u.
    const double lambda_osqp = dual(fixed_u_constraint_index);
    result.lambda_u = lambda_osqp;
    result.dQ_du_rad = -lambda_osqp;

    // Policy output and training data use degrees, while the QP action variable is radians.
    // dQ/d(deg) = dQ/d(rad) * pi/180.
    result.dQ_du_deg = result.dQ_du_rad * M_PI / 180.0;

    result.valid = std::isfinite(result.objective) &&
                   std::isfinite(result.dQ_du_rad) &&
                   std::isfinite(result.dQ_du_deg);
...
ExactQResult MpcController::evaluateExactQWithGradient(const MpcState& state,
                                                       float velocity,
                                                       double u0_fixed_deg) {
    ExactQResult result;

    if (!initialized_) {
        std::cerr << "[MPC][ExactQGrad] Not initialized!" << std::endl;
        return result;
    }
    ...
    if (state.curvature.size() < static_cast<size_t>(N_)) {
        std::cerr << "[MPC][ExactQGrad] Curvature vector too small!" << std::endl;
        return result;
    }
    ...
    const double u0_fixed_rad = u0_fixed_deg * M_PI / 180.0;
    return solveQPExactQWithGradient(x0, v_k, u0_fixed_rad);
}
